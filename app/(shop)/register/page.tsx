    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          ...
          toast.error(data.message || 'Mohon lengkapi semua data');
        } else {
      ...
      <div className="bg-white p-8 rounded-3xl shadow-soft border border-border/60 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input id="name" type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="rounded-xl" placeholder="Nama Anda" />
          </div>
          <div className="space-y-1.5">
