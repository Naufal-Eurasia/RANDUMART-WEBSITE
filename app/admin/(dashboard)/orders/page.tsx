...
const statusColors: Record<string, string> = {
  PENDING: 'bg-[#B8791F] text-white',
  PAID: 'bg-[#28331F] text-[#E7DCC3]',
  PROCESSING: 'bg-[#E7DCC3] text-[#28331F]',
        ...
        {['ALL', 'PENDING', 'PAID', 'PROCESSING', 'SHIPPED', 'COMPLETED'].map(tab => (
          <button
            ...
            className={`px-4 py-2 rounded-full text-sm font-semibold whitespace-nowrap transition-colors ${
              activeTab === tab
                ? 'bg-[#28331F] text-[#E7DCC3] shadow-sm'
                ...
                filteredOrders.map((o) => (
                  <tr key={o.id} className="hover:bg-muted/30">
                    <td className="px-6 py-4">
                      <button onClick={() => openDetail(o)} className="font-semibold text-[#B8791F] hover:underline flex items-center gap-1.5">
            ...
            <div className="space-y-6">
              <div className="grid sm:grid-cols-2 gap-4 text-sm">
                <div className="p-4 bg-muted/50 rounded-2xl border border-border/50">
                  <h4 className="font-semibold mb-3 flex items-center gap-1.5"><MapPin className="w-4 h-4 text-[#B8791F]"/> Info Pengiriman</h4>
                ...
                <div className="p-4 bg-muted/50 rounded-2xl border border-border/50 space-y-2">
                  ...
                  <div className="flex justify-between"><span className="text-muted-foreground">Total Bayar</span> <span className="font-bold text-[#B8791F]">{formatRupiah(Number(selectedOrder.totalAmount))}</span></div>
                </div>
              </div>

              <div>
                <h4 className="font-semibold mb-3 flex items-center gap-1.5"><Package className="w-4 h-4 text-[#B8791F]"/> Item Dibeli ({selectedOrder.items.length})</h4>
